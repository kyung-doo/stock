...
// 상승률 계산
function pctChange(a, b) { return b === 0 ? 0 : (a / b - 1); }
...
async function getTopByVolume(limit = 100, months = 1) {
  ...
  const withVolume = results.filter(Boolean).sort((a, b) => b.avgVol - a.avgVol);
...
app.get('/top-risers', async (req, res) => {
  ...
  const kospiMA200 = kospiDaily.slice(-window).reduce((s,d)=>s+d.close,0)/window;
  ...
  const tasks = symbols.map(async ({symbol, name}) => {
    ...
    for (let i = 1; i < kRecent.length; i++) {
      if (kRecent[i].close < kRecent[i-1].close) kospiDownDates.push(kRecent[i].date);
    }
    let coDown = 0;
    for (let i = 1; i < sd.length; i++) {
      if (kospiDownDates.includes(sd[i].date) && sd[i].close < sd[i-1].close) coDown++;
    }
    const coDownRate = kospiDownDates.length ? coDown / kospiDownDates.length : 0;
